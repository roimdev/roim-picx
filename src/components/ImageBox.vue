<template>
    <div class="group relative bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-md transition-all duration-300"
        v-if="!imageError">
        ...
        <div class="relative w-full aspect-[3/2] sm:aspect-square overflow-hidden bg-gray-50">
            ...
            <div
                ...
                <template v-if="mode === 'uploaded'">
                    ...
                    <el-tooltip content="预览" placement="top" :show-after="500">
                        <button
                            class="w-10 h-10 rounded-full bg-white text-gray-700 hover:text-green-600 hover:scale-110 transition-all flex items-center justify-center shadow-lg"
            ...
            <div class="absolute bottom-0 left-0 right-0 p-2 bg-gradient-to-t from-black/60 to-transparent flex sm:hidden items-center justify-end gap-3 z-10"
                ...
                <button
                    class="w-8 h-8 rounded-full bg-white/90 text-gray-700 active:bg-green-50 active:text-green-600 transition-all flex items-center justify-center shadow-lg backdrop-blur-sm"
                    @click.stop="emit('preview')">
